...
def measure_inference_latency(
    model, device, input_size=(1, 3, 32, 32), num_samples=100, num_warmups=10
):
...
def create_model(num_classes=10):
    ...
    # CIFAR-10 adaptation: 32x32 输入，原版 stem (kernel=7, stride=2, maxpool)
    # 会快速把图缩到 7x7，丢失信息。改成 3x3 conv + stride 1 + 去 maxpool
...
def main():
    ...
    # TODO 这里已经有准备好进行QAT的准备了
    # 实际就是在这里进行插入fake quant 相关的量化节点
    # torch.quantization.prepare_qat(quantized_model, inplace=True)
    ...
    # 在这里自动将fakequant转换成 插入的量化节点于算子融合
    ...
    print(
        "FP32 CPU Inference Latency: {:.2f} ms / sample".format(
            fp32_cpu_inference_latency * 1000
        )
    )
    print(
        "FP32 CUDA Inference Latency: {:.2f} ms / sample".format(
            fp32_gpu_inference_latency * 1000
        )
    )
    print(
        "INT8 CPU Inference Latency: {:.2f} ms / sample".format(
            int8_cpu_inference_latency * 1000
        )
    )
    print(
        "INT8 JIT CPU Inference Latency: {:.2f} ms / sample".format(
