...
__global__ void reduce_v3(float *d_in, float *d_out) {
    ...
    // gtid + blockSize = 2; when tid = 1, gtid = 1, gtid + blockSize = 3 so, we can understand L18,
    ...
    //! 利用了数据都放在寄存器，实际的多load了线程，底下的参数少了一倍，进而少了一次循环
    //* 但是不能一直这么写，因为实际的寄存器是有限的，不好这么去一直进行手写加
    smem[tid] = d_in[gtid] + d_in[gtid + blockSize];
    __syncthreads();

    // 同v2:
    // 从之前的当前线程ID加2*线程ID位置然后不断加上*2位置上的数据，改成不断地对半相加，以消除bank
    // conflict 此时一个block对d_in这块数据的reduce sum结果保存在id为0的线程上面
