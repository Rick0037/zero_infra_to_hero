...
// 注意:
// v0-v5里面kernel得到的是各个block负责范围内的总和，要想得到最终的和，需要把各个block求得的总和再做reduce
// sum v6: multi-block reduce final result by two pass latency: 1.815ms
 ...
 */
template <int blockSize>
__global__ void reduce_v6(float *d_in, float *d_out, int number) {
    // 必须编译时的
    __shared__ float smem[blockSize];
    //
    unsigned int tid = threadIdx.x;
    unsigned int gid = threadIdx.x + blockDim.x * blockIdx.x;
    unsigned int total_thread_number = blockDim.x * gridDim.x;
    float sum = 0.0f;
    // 一个一个 total_thread_number 在滚动的往后加
    for (int i = gid; i < number; i += total_thread_number) {
        sum += d_in[i];
    }
    // 一个线程干两个活的终极版
    smem[tid] = sum;
